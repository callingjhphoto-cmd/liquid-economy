                      ...
                      <div className="flex flex-col sm:flex-row sm:items-end gap-3 sm:gap-6">
                        ...
                        >
                          <AreaChart data={Object.entries(data.historicalData).sort(([a], [b]) => a.localeCompare(b)).map(([d, v]) => ({ date: d.slice(5), value: v }))} accessibilityLayer={true}>
                            ...
                            <YAxis tick={{ fontSize: 10, fill: '#9ca3af' }} tickLine={false} axisLine={false} width={54} domain={['auto', 'auto']} tickFormatter={v => data.unit ? `${v} ${data.unit}` : String(v)} />
                            <Tooltip contentStyle={{ background: '#1e293b', border: 'none', borderRadius: 8, fontSize: 11, color: '#f1f5f9' }} labelStyle={{ color: '#f1f5f9' }} itemStyle={{ color: '#f1f5f9' }} formatter={v => [`${v}${data.unit ? ' ' + data.unit : ''}`, data.label]} />
